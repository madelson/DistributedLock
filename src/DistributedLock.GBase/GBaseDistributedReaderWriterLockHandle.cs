...
/// </summary>
public abstract class GBaseDistributedReaderWriterLockHandle : IDistributedSynchronizationHandle
{
    // forbid external inheritors
    internal GBaseDistributedReaderWriterLockHandle() { }
...
internal sealed class GBaseDistributedReaderWriterLockNonUpgradeableHandle : GBaseDistributedReaderWriterLockHandle
{
    private IDistributedSynchronizationHandle? _innerHandle;

    internal GBaseDistributedReaderWriterLockNonUpgradeableHandle(IDistributedSynchronizationHandle? handle)
    {
...
/// </summary>
public sealed class GBaseDistributedReaderWriterLockUpgradeableHandle : GBaseDistributedReaderWriterLockHandle, IInternalDistributedLockUpgradeableHandle
{
    ...
    internal GBaseDistributedReaderWriterLockUpgradeableHandle(IDistributedSynchronizationHandle innerHandle, IDbDistributedLock @lock)
    {
    ...
    ValueTask<bool> IInternalDistributedLockUpgradeableHandle.InternalTryUpgradeToWriteLockAsync(TimeoutValue timeout, CancellationToken cancellationToken)
    {
        ...
        {
            var upgradedHandle =
                await contents.@lock.TryAcquireAsync(timeout, GBaseDbmsLock.UpgradeLock, cancellationToken, contextHandle: contents.innerHandle).ConfigureAwait(false);
